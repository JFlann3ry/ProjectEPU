...
async def extras_checkout(
    ...
    if event_id is None:
        ...
        if event_id:
            try:
                ...
                resolved_code = None
    if not code:
        raise HTTPException(status_code=400, detail="Missing extra code")
    addon = (
        db.query(AddonCatalog)
        .filter(
    ...
    if amount_cents <= 0:
        raise HTTPException(status_code=400, detail="Extra price invalid; please contact support")
    ...
    try:
        ...
        suffix = ""
    # If this is the additional_event addon, redirect user to the event creation page on success
    if code == "additional_event":
        success_url = f"{base}/events/create{suffix}"
    else:
