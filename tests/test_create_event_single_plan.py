...
def seed_user_with_single_plan(db):
    ...
    p = (
        db.query(Purchase)
        .filter(Purchase.UserID == u.UserID, Purchase.PlanID == plan.PlanID)
        .first()
    )
    if not p:
        p = Purchase(
...
def test_create_event_shows_purchase_link():
    ...
    try:
        ...
        # Create a valid session and attach it to the test client to simulate
        # authentication
        sess = create_session(db, user_id=int(getattr(u, "UserID")))
        client.cookies.set("session_id", str(sess.SessionID))
        # For test simplicity, call GET /events/create and expect the template
        # to include the purchase link
        r = client.get("/events/create")
        if r.status_code == 302 and "location" in r.headers:
            r = client.get(r.headers["location"])
        assert r.status_code == 200
        text = r.text
        # We expect either the explicit purchase link or that the user was
        # redirected to billing/pricing
        lowered = text.lower()
        assert (
...
def test_create_event_page_uses_extracted_page_script(monkeypatch):
    db = SessionLocal()
    try:
        u, plan, ev = seed_user_with_single_plan(db)
        monkeypatch.setattr(
            "app.services.billing_utils.get_active_plan",
            lambda _db, _user_id: (SimpleNamespace(Code="single"), {}),
        )
        sess = create_session(db, user_id=int(getattr(u, "UserID")))
        client.cookies.set("session_id", str(sess.SessionID))
        r = client.get("/events/create")
        assert r.status_code == 200
        assert "js/pages/create_event.js" in r.text
        assert "const typeSelect = document.getElementById('type');" not in r.text
    finally:
        db.close()
