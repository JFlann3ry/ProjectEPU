...
def test_owner_upload_rejects_disallowed_type(db_session, client):
    ...
    before = db_session.query(FileMetadata).filter(FileMetadata.EventID == ev.EventID).count()
    r = client.post(f"/events/{ev.EventID}/upload", files=files)
    # Should still redirect, but not create metadata
    assert r.status_code in (200, 303)
    after = db_session.query(FileMetadata).filter(FileMetadata.EventID == ev.EventID).count()
    assert after == before
