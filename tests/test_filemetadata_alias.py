...
def test_fileid_synonym(db_session):
    ...
    u = User(FirstName='FM', LastName='Test', Email='fmfiletest@example.test', HashedPassword='x', IsActive=True)
    db_session.add(u)
    db_session.flush()

    e = Event(EventTypeID=None, UserID=u.UserID, Name='T', Code='FMTEST', Password='pw', TermsChecked=True)
    db_session.add(e)
    db_session.flush()

    # Create minimal FileMetadata row referencing the event we just created
    fm = FileMetadata(EventID=e.EventID, FileName='x.jpg', FileType='image/jpeg', FileSize=100)
    db_session.add(fm)
    db_session.commit()
