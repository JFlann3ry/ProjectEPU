...
def test_event_gallery_order_respects_ordinal(db_session, client: TestClient):
    ...
    u = db_session.query(User).filter(User.Email == "gallery_test@example.test").first()
    if not u:
        u = User(
            FirstName="G",
            LastName="T",
    ...
    f1 = FileMetadata(EventID=ev.EventID, FileName="a.jpg", FileType="image/jpeg", FileSize=100)
    f2 = FileMetadata(EventID=ev.EventID, FileName="b.jpg", FileType="image/jpeg", FileSize=100)
    f3 = FileMetadata(EventID=ev.EventID, FileName="c.jpg", FileType="image/jpeg", FileSize=100)
